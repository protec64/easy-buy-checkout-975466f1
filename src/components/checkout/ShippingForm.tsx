...
interface ShippingFormProps {
  values: { cep: string; street: string; number: string; complement: string; neighborhood: string; city: string; state: string; reference: string; };
      ...
      <div className="space-y-3">
        <div className="max-w-[200px]">
          ...
          <div className="relative">
            <Input id="cep" placeholder="00000-000" value={values.cep}
              onChange={(e) => handleCepChange(e.target.value)} maxLength={9} className={fc("cep")} />
          ...
          <>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_120px]">
              <div>
                ...
                <Input id="street" value={values.street} onChange={(e) => onChange("street", e.target.value)} className={fc("street")} />
              ...
              <div>
                ...
                <Input id="number" value={values.number} onChange={(e) => onChange("number", e.target.value)} className={fc("number")} />
            ...
            <div>
              ...
              <Input id="complement" placeholder="Apto, bloco, etc." value={values.complement} onChange={(e) => onChange("complement", e.target.value)} />
            </div>

            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div>
                <Label htmlFor="neighborhood" className="text-xs text-muted-foreground">Bairro *</Label>
                <Input id="neighborhood" value={values.neighborhood} onChange={(e) => onChange("neighborhood", e.target.value)} className={fc("neighborhood")} />
              ...
              <div>
                ...
                <Input id="city" value={values.city} onChange={(e) => onChange("city", e.target.value)} className={fc("city")} />
                {errors.city && <p className="mt-1 text-xs text-destructive">{errors.city}</p>}
              </div>
              <div>
                <Label htmlFor="state" className="text-xs text-muted-foreground">UF *</Label>
                <Input id="state" placeholder="SP" maxLength={2} value={values.state}
                  onChange={(e) => onChange("state", e.target.value.toUpperCase().slice(0, 2))} className={fc("state")} />
                {errors.state && <p className="mt-1 text-xs text-destructive">{errors.state}</p>}
              </div>
            </div>

            <div>
              <Label htmlFor="reference" className="text-xs text-muted-foreground">Referência</Label>
              <Input id="reference" placeholder="Próximo a..." value={values.reference} onChange={(e) => onChange("reference", e.target.value)} />
            </div>
          </>
        )}
      </div>
