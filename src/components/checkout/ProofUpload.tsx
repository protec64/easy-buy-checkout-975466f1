...
const ProofUpload = ({ paymentId, email, cpf, orderId, onUploadSuccess }: ProofUploadProps) => {
  ...
  const validateFile = (f: File): boolean => {
    if (!ACCEPTED.includes(f.type)) { setError("Formato não aceito. Use JPG, PNG ou PDF."); return false; }
    if (f.size > MAX_SIZE) { setError("Arquivo muito grande. Máximo 10MB."); return false; }
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      await uploadPaymentProof(file, { payment_id: paymentId, email, cpf, order_id: orderId, note: note || undefined });
      setUploaded(true);
      onUploadSuccess?.();
    } catch (err: any) {
  ...
  if (uploaded) {
    return (
      <div className="rounded-lg border border-checkout-trust/30 bg-checkout-trust/5 p-4 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">Vamos validar e te avisar por email/WhatsApp.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 rounded-lg border border-border bg-checkout-highlight p-4">
      <p className="text-sm font-medium text-foreground">Já pagou? Envie o comprovante</p>
      ...
      {file && (
        <>
          <Textarea placeholder="Observação do pagamento (opcional)" value={note}
            onChange={(e) => setNote(e.target.value)} rows={2} className="text-sm" />
          <Button onClick={handleUpload} disabled={uploading} className="w-full gap-2 bg-primary text-primary-foreground hover:bg-primary/90">
            {uploading ? (<><Loader2 className="h-4 w-4 animate-spin" />Enviando...</>) : (<><Image className="h-4 w-4" />Enviar comprovante</>)}
          </Button>
        </>
      )}
    </div>
