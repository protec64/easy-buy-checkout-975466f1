...
interface PaymentSectionProps {
  ...
  pixLoading: boolean;
  onGeneratePix: () => void;
  cardValues?: { cardNumber: string; cardName: string; expiry: string; cvv: string; installments: string; };
...
const PaymentSection = ({ pixData, pixLoading, onGeneratePix, total, email, cpf, fullName, phone, city, state, zipCode, items }: PaymentSectionProps) => {
  return (
    <div className="rounded-xl border border-border bg-card p-4 sm:p-5 checkout-shadow">
      ...
      <div className="mb-4 flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2.5">
        ...
        <span className="ml-auto rounded bg-accent/20 px-1.5 py-0.5 text-[10px] font-medium text-accent">Aprovação instantânea</span>
      </div>
